...
import { MessageSquare, Image as ImageIcon, FileText, Check, UserCheck, Calendar } from 'lucide-react';

const { groups } = generateMockData();

export default function BroadcastBuilder() {
  const [selectedAudiences, setSelectedAudiences] = useState<string[]>(['VIP Customers']);
  ...
  const duplicateDeduction = selectedAudiences.length > 1 ? (selectedAudiences.length * 42) : 0;
  const finalReach = baseCount - duplicateDeduction;

  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-zinc-50">Campaign Composer</h1>
        <p className="text-sm text-zinc-400 mt-1">Design hyper-personalized outreach structures with sub-second live rendering.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
        <div className="xl:col-span-2 space-y-6">
          <div className="bg-zinc-900/30 border border-zinc-800/80 rounded-2xl p-6 space-y-4">
            <h3 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
              <UserCheck className="h-4 w-4 text-emerald-400" /> 1. Select Targeted Clusters
            </h3>
            ...
            <div className="p-3.5 bg-zinc-950 rounded-xl border border-zinc-800/60 grid grid-cols-3 text-center divide-x divide-zinc-800">
              <div>
                <p className="text-[10px] font-medium text-zinc-500 uppercase">Gross Raw Contacts</p>
                <p className="text-base font-bold text-zinc-300 mt-1 font-mono">{baseCount}</p>
              </div>
              <div>
                <p className="text-[10px] font-medium text-zinc-500 uppercase">Cross-Deduplicated</p>
                <p className="text-base font-bold text-rose-400 mt-1 font-mono">-{duplicateDeduction}</p>
              </div>
              <div>
                <p className="text-[10px] font-medium text-emerald-400 uppercase">Net Realized Reach</p>
                <p className="text-base font-bold text-emerald-400 mt-1 font-mono">{finalReach}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/30 border border-zinc-800/80 rounded-2xl p-6 space-y-4">
            <h3 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
              <MessageSquare className="h-4 w-4 text-blue-400" /> 2. Content Formulation Workspace
            </h3>
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center justify-center space-x-1.5 py-2 text-xs font-medium rounded-lg transition ${
                    mediaType === t.id ? 'bg-zinc-800 text-zinc-100 shadow' : 'text-zinc-500 hover:text-zinc-300'
                  }`}
                >
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs text-zinc-500 px-1">
                <span>Dynamic Syntax Template Variable: <code className="bg-zinc-800 text-zinc-300 px-1 py-0.5 rounded">{"{{FirstName}}"}</code></span>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center pt-2 gap-4">
              <div className="flex items-center space-x-2 text-xs text-zinc-500">
                ...
                <span>Scheduling Zone: UTC +05:30 (IST Asia/Kolkata)</span>
              </div>
        ...
        <div className="flex justify-center xl:sticky xl:top-6">
          <div className="w-[310px] h-[610px] bg-zinc-950 border-[6px] border-zinc-800 rounded-[40px] shadow-2xl relative overflow-hidden flex flex-col">
            ...
            <div className="bg-[#075e54] pt-5 pb-3 px-4 text-white flex items-center space-x-2">
              <div className="h-7 w-7 rounded-full bg-zinc-300/30 flex items-center justify-center font-bold text-xs">V</div>
              <div>
                <p className="text-xs font-semibold tracking-tight">Vortex Outbound Gateway</p>
                <p className="text-[9px] text-emerald-200/80">Official Verified Cloud Account</p>
              </div>
            </div>

            <div className="flex-1 bg-[#ebe5df] p-3 space-y-3 overflow-y-auto relative pattern-dots">
              <div className="mx-auto text-center my-1">
                <span className="bg-white/80 backdrop-blur-sm text-[9px] font-semibold text-zinc-500 px-2 py-0.5 rounded shadow-sm uppercase tracking-wide">Today</span>
              </div>
              ...
              <div className="max-w-[85%] bg-white rounded-lg p-2.5 shadow-sm text-zinc-800 text-[11px] font-normal leading-relaxed relative ml-1 border-l-2 border-emerald-500">
                ...
                {mediaType === 'pdf' && (
                  <div className="w-full bg-zinc-100 p-2 rounded mb-2 flex items-center space-x-2 border border-zinc-200">
                    ...
                    <span className="text-[9px] font-medium text-zinc-600 truncate">commercial_invoice.pdf</span>
                  </div>
                )}
